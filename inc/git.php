...
/*-----------------------------------------------------------------------------
 ...
 |
 | File inc/git.php
 |    Summary: Provides the mechanism for connecting to a GitHub repository,
...
class GithubRepository {
	...
	private $bitsand_path = '';

	/**
	 * Establish a connection to a Github repository
	 * @param type $repository_path The repository to use in the format
	 * <UserName>/<RepoName>
	 * @param string $source [Optional] Currently only accepts github
	 */
	public function __construct($repository_path, $source = 'github') {
		$this->bitsand_path = dirname(dirname(__FILE__) . '../') . DIRECTORY_SEPARATOR;
		...
		switch ($source) {
			case 'github':
				$this->path = 'https://api.github.com/repos/' . $repository_path;
				break;
			default:
				return false;
		}
